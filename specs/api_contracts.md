...
When an authenticated Mac needs a fresh session, reconnect is deliberately narrower
than first sign-in. The native app keeps the existing Keychain session in place while it
starts a new PKCE email flow for the verified saved email. The callback exchange must
resolve to the same Supabase user ID before it can replace that session; a different
account is rejected. A failed or abandoned reconnect therefore cannot silently sign the
Mac into another library or discard locally queued originals.

Each installed Mac also owns one independent Keychain device secret. After account
authentication, the client registers only the secret's SHA-256 fingerprint through
`register_mac_device`; the raw secret never enters Postgres. Every
`ingest_native_capture` call supplies the installation ID and secret alongside the
verified Supabase JWT. The RPC requires all three to resolve to the same active owner,
records `mac_device_id` on the screenshot, and updates the device's last-capture time.
`revoke_mac_device` is owner-scoped and immediately blocks later registrations from that
installation even while an old account access token remains valid. A revoked client
keeps its local originals and requires same-account reconnect before it creates a fresh
...
`source` ∈ `hotkey_region | hotkey_window | hotkey_fullscreen | drag | clipboard | web_upload | extension`. (`capture_kind` is implicitly `screenshot` in v1; field reserved per ../04_MVP_SCOPE.md.) `screenshot_id` is generated by the capture client and reused across every retry. There is deliberately no `user_id`: the function derives ownership from the bearer JWT, verifies that the owner segment immediately after the `originals/` bucket prefix equals `auth.uid()`, and verifies that the final filename equals `<screenshot_id>.png` before inserting anything. Native Quick Access may persist an optional `project_id` into the durable queue before its upload hold closes. The authenticated RPC independently requires that project to be active and owned by `auth.uid()`, records `assignment_source: manual`, and rejects a retry that attempts to change a prior manual choice. When no project was chosen, a retry remains idempotent after AI classification has assigned one.
...
## Chrome extension (direct account delivery added 2026-08-12)

The configured Capso origin remains the only host the extension can reach. Visible `activeTab` capture produces a ≤1600px JPEG original; area mode maps a CSS-pixel selection onto the exact retina viewport before applying the same cap; full-page mode scrolls and stitches into a bounded ≤1600×12000 JPEG. All modes produce an 800px WebP thumbnail and SHA-256 fingerprint in the MV3 service worker. Raw retina PNGs/tiles never cross the wire, and every original must fit the 2.25 MiB direct-ingest boundary.

**Pairing.** A signed-in `/extension` page derives a high-entropy account-scoped device code and registers only its SHA-256 hash in `extension_devices` through the caller's verified Supabase JWT and owner RLS. The raw code stays in the browser profile. Switching accounts derives a different code; revoking the device row disables that code without affecting another device. The copied-code setup remains a transitional UX until a stable Web Store extension ID makes a verified one-click handoff possible.

**Project choice.** `GET /api/extension/projects` accepts the paired code only in `x-capso-device`, proxies to the same Edge boundary and returns at most 50 active projects for the resolved owner. A capture may carry one selected `projectId`; the service-role RPC independently verifies owner + active state, stores it as a `manual` assignment, and background classification is forbidden from replacing an existing explicit assignment.

**Delivery.** The durable IndexedDB outbox calls `POST {origin}/api/extension/ingest`. Next.js applies the browser-origin/body boundary and proxies to `/functions/v1/v1-extension-ingest` with the public anon key only. The Edge Function hashes the device code, resolves one non-revoked owner with its service-role connection, writes immutable `originals/{owner}/{id}.jpg` and `thumbs/{owner}/{id}.webp` objects, then calls the service-role-only `ingest_extension_capture` RPC. That RPC re-resolves ownership from the device row and atomically inserts the screenshot plus one processing job.

`201` is a new capture; `200` with `deduped: true` is an idempotent retry. The extension retires its local item only when `screenshot_id` exactly equals the client-generated ID. `401/403`, `409`, `429` and `5xx` remain queued. Only `404/501` selects the old `/api/ingest` relay, allowing an updated extension to work against an older or local-only Capso deployment without treating auth or transient failures as permission to race two delivery paths.

The legacy relay remains migration-only: it hands addressed items to an open same-origin Capso tab and waits for an explicit acknowledgement. It can be removed after all supported deployments expose direct ingest and the migration window closes.
...
Dedupe hit returns `200` with the requested id and `"deduped": true`. Side effects: inserts or resolves the requested `screenshots` identity (status `processing`) and enqueues `process_capture` only when work is not already complete. Extension thumbnails arrive with the original; native thumbnail generation remains surface-specific.

The response `screenshot_id` must equal the request `screenshot_id`, including on a
dedupe response. Mac and extension queues mark an item remotely persisted only after
that exact acknowledgement; a different id is a protocol failure and local bytes remain queued.
