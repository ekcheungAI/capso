fn main() {
    ...
    println!("cargo:rerun-if-env-changed=CAPSO_AUTH_SITE_URL");
    ...
    match (url.as_deref(), key.as_deref()) {
        (None, None) => {
            if std::env::var("CAPSO_AUTH_SITE_URL").is_ok() {
                panic!("native auth website requires the native Supabase public configuration");
            }
        }
        (Some(url), Some(key))
    ...
    if let Ok(site) = std::env::var("CAPSO_AUTH_SITE_URL") {
        if !site.starts_with("https://") || site.contains(['?', '#', '@']) {
            panic!(
                "native auth website must be one HTTPS origin without credentials or query data"
            );
        }
    }
