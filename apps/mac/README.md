...
The current app intentionally has no production transport or wake source wired to that coordinator. It does not yet authenticate, upload, monitor connectivity, create server processing jobs, or run background AI; those remain DUR-01b2/AI-01. The fake-transport proofs are not the AC-OFF-01 offline drill. Until those paths and their manual three-capture restart/reconnect drill pass, “sync pending” means safely retained on this Mac, not remotely stored.
...
Rust and the shared Zod package consume one strict fixture plus shared negative timestamp/message boundaries. This is a compiled contract seam, not production connectivity: there is no deep-link registration or command, browser sign-in route, Supabase code exchange, Keychain session, HTTP upload, connectivity wake source, or server worker yet. The current anonymous browser library also cannot be silently transferred to a Mac identity. Production auth/linking is an owner decision; until it is made and the real adapter passes, Capso does not authenticate or upload from the desktop and cannot run background learning with the browser closed.
