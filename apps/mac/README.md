...
Screen Recording status is checked without prompting at startup and whenever the popover regains focus. Region capture always uses macOS's live native area picker and remains fully available without Screen Recording permission. Window and fullscreen capture are blocked before `/usr/sbin/screencapture` can run, regardless of whether the request came from a shortcut, tray action, or direct Tauri command; a shortcut or tray attempt opens the visible guidance instead of saving blank pixels. The OS prompt only follows an explicit **Grant access** action and is attempted at most once per app session. A denied request becomes an **Open settings** action. Choosing **Use Area only** is durable across relaunches and suppresses the global permission nag while leaving honest permission labels on Window and Full Screen.
