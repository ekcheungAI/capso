...
import { EXTENSION_DEVICE_ID } from "@/lib/extension-device";
...
export async function GET(request: Request) {
  ...
  const { data, error } = await account.client
    ...
    .select("id,label,created_at,last_seen_at,last_capture_at")
    .eq("user_id", account.userId)
    .is("revoked_at", null)
    .order("created_at", { ascending: false });
  if (error) return Response.json({ error: "devices could not be loaded" }, { status: 502 });
...
export async function DELETE(request: Request) {
  ...
  const id = body && typeof body === "object" && "id" in body
    ...
    : null;
  if (typeof id !== "string" || !EXTENSION_DEVICE_ID.test(id)) {
  ...
  const { data, error } = await account.client
    ...
    .select("id")
    .maybeSingle();
  if (error) return Response.json({ error: "device could not be disconnected" }, { status: 502 });
