...
| M3 | Background upload pipeline | Capture → local queue → Supabase Storage + `screenshots` row; survives offline (retry via queue); capture is never blocked or lost by network | J1 / UC4 |
...
| M10 | Chrome extension capture (D11, added 2026-07-31) | MV3 extension captures the visible tab via hotkey or toolbar button and posts it into the same ingest pipeline. **Browser tabs only** — native app windows still require M1's Mac capture, so this complements it | J1 / UC2, UC3 |
...
| M11 | Starter kits + project shelves (added 2026-08-01) | First run asks what kind of work the user screenshots for and creates that role's projects with descriptions (03 §starter kits); those descriptions feed the classifier's candidate list (07 §1). The library groups into per-project shelves by default — droppable, with a "N waiting" chip per project and a one-line ledger of what has been captured, filed and is still waiting | J3, J6 / UC2, UC3, UC5 |
...
| `source_url` / hostname as classifier signal (added 2026-08-01) | Cheap accuracy win that *reduces* vision-model dependence (J3 / UC2, UC3) — the extension already captures the tab URL and Capso discards it. But the extension→ingest path must be reliable before adding signal to it (25 §2.3) | Immediately after extension ingest is verified working in a real Chrome |
| Session / flow grouping (added 2026-08-01) | Captures from one browsing session kept linked as a sequence (J6 / UC3, competitor funnel steps). Needs a stable ingest pipeline; new grouping UI is not loop-critical | After M1 + M3 land and the owner captures a real multi-step flow |
| `has_pending_action` resurface ranking (added 2026-08-01) | Derived OCR signal (date/deadline/price/CTA) that ranks the existing Resurface shelf only — **no task list, no checkboxes, no due dates**, per the non-goal at 01 (J9 / UC5). Needs weeks of capture data, same as digests. Note Apple's Add to Calendar already covers the single-screenshot version (25 §2.5) | With the weekly-digest work (row above) |
| Multi-project membership (added 2026-08-01) | One capture legitimately belongs to two threads (competitor pricing → "Competitor research" *and* "Pricing redesign"). Adds no manual filing, so it is **not** the rejected tags/folders tripwire below — but it is a model change (J6 / UC3) | Owner hits the two-projects problem >2×/week in daily use |
| Public changelog (added 2026-08-01) | Trust signal, zero loop value — but the category is a graveyard and "still maintained" is a stated buying criterion (25 §2.4) | Before first external tester (18 Q5) |
