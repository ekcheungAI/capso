...
| ~~`source_url` / hostname as classifier signal~~ **— shipped, no longer deferred** | Added 2026-08-01, found already implemented 2026-08-02: commit `3c9f981` wired `pageUrl`/`pageTitle` end to end (extension → ingest → capture → classify), and `reclassify.ts` re-reads with the same page context. Row kept rather than deleted so the reversal is legible | Done |
