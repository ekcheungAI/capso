...
## Loop 36 — Interactive Quick Access core actions
**Date:** 2026-08-08 · **Phase:** P2 / OVL-01b1 · **Outcome:** done; partial OVL-01 and CAP-02 evidence

Objective: Turn the approved native thumbnail into a safe interactive surface with Copy, Save As, Close, and an eight-second hover/action-paused auto-dismiss timer, without allowing a delayed action to mutate a newer capture or lose durable pixels.

The 252×194 nonfocusable overlay now accepts deliberate pointer interaction. Copy reuses the exact persisted PNG AppKit path; Save As uses the scoped native dialog and streams to a same-directory temporary file before atomic rename; Close and timeout hide only the exact active capture. Hovering or opening an action pauses the remaining timeout instead of restarting it, and an exhausted timeout is one-shot even when native dismiss fails. No dead Annotate, drag, or history buttons were added.

Two concurrency layers protect replacement captures. React assigns every presentation and action a generation, so an old dismiss/copy/save response or `finally` cannot clear the new preview, overwrite its notice, or re-enable controls during its action. Rust registers every new capture in a shared clipboard generation before asynchronous reading, then revalidates under the same mutex at the main-thread AppKit mutation point. An old manual copy therefore commits before the new registration and is followed by new pixels, or is rejected without touching the pasteboard. Save As also resists same-path, hard-link, and symlink-style destination alias truncation.

Verification: final Rust suite **59/59 passed**; Mac action/timer suite **6/6**, web **78/78**, and extension **4/4** passed. `cargo fmt --check`, strict Clippy across all targets/features, root typecheck/lint, brand/capture checks, all production builds, frozen offline install, loop validator (**67 checks + 7 malformed fixtures**), and `git diff --check` passed. A fresh debug Tauri `Capso.app` bundle was produced. Direct 252×194 dark-mode interaction QA confirmed full bounds, visible/enabled controls, Copy/Save hover states, no clipping, and zero console warnings/errors; the approved OVL-01a shell retains prior light-mode evidence.

Checker: the initial review rejected a stale React dismiss/action continuation and a delayed old AppKit copy that could overwrite newer pixels. Repair attempt 1 added presentation/action generations and clipboard registration/commit linearization with adversarial ordering tests. The independent Checker then **APPROVED** with no remaining P0–P2 findings. Implementation commit: `8923e90`.

OVL-01 and CAP-02 remain IN_PROGRESS. Annotate, native drag-out, and durable recent restore remain OVL-01b2. Native Copy/Save dialog behavior, focus preservation, mixed-scale multi-display placement, perceived latency, and general-pasteboard behavior still require manual QA. The existing `.app`-suffixed identifier warning remains PKG-01a. No app or CleanShot capture mode was launched, and no clipboard or CleanShot setting was changed. Next loop: OVL-01b2 completion actions.

## Loop 37 — Durable Recent Captures restore
**Date:** 2026-08-08 · **Phase:** P2 / OVL-01b2a + partial HIS-01 · **Outcome:** done; partial OVL-01 and HIS-01 evidence

Objective: Make the five newest trustworthy native captures survive a Capso relaunch and reopen from a native tray submenu without silently changing the pasteboard, trusting stale paths, or allowing a history action to overtake a new capture.

Capso now scans the Application Support `captures/` directory at startup and after every durable capture. It accepts only regular, direct-child, canonical UUID PNG files that pass both signature and full pixel decode, sorts them newest-first with a UUID tie-break, and builds a native **Recent Captures** submenu containing at most five exact UUID action IDs. Missing storage is an honest empty state. A selection re-resolves the UUID against disk before restoring, so paths, indices, corrupt replacements, symlinks, nesting, and stale entries are rejected.

History restore targets the display containing the cursor and presents the existing Quick Access overlay as **Recent capture / Ready to copy**. The pasteboard stays unchanged until **Copy**. Native monotonic presentation IDs make repeated callbacks safe even when the same UUID path is restored twice. Clipboard ticket activation and overlay publication are one lock-held transaction; a fresh capture either registers first and rejects history or waits until history is visible, then supersedes the clipboard identity and overlay together. Fresh capture ownership also remains live through its own overlay publication.

Verification: Rust **74/74**, Mac action/timer **6/6**, web **78/78**, and extension **4/4** tests passed. `cargo fmt --check`, strict Clippy across all targets/features, root typecheck/lint, brand/capture checks, all production builds, a fresh debug Tauri `Capso.app`, loop validator (**67 checks + 7 malformed fixtures**), and `git diff --check` passed. The 252×194 history preview passed native-size browser QA with visible Copy/Save/Close controls and zero console warnings. Native tray/relaunch interaction was not claimed: Accessibility inspection could not reliably expose menu item labels, so relaunch population, repeated selection, cursor-display placement, focus, and real pasteboard behavior remain manual evidence.

Checker: the initial review rejected same-path callbacks keyed only by path, early fresh-capture ownership release, and signature-only PNG validation. Repair attempt 1 added presentation IDs, ownership through overlay publication, and full decode. The second review found the inverse race: a history restore released clipboard ownership before publishing its overlay, allowing a fresh capture to register first and then be visually replaced by delayed history. Repair attempt 2 made history activation/publication atomic and added a deterministic blocking interleaving test. The independent Checker then **APPROVED** with no remaining P0–P2 findings. Implementation commit: `8bd0888`.

OVL-01 and HIS-01 remain IN_PROGRESS. Annotate, native drag-out, thumbnails/full-library deep links, durable upload queue, and the native evidence above remain. The existing `.app`-suffixed identifier warning remains PKG-01a; the bundle is not ready for distribution. The installed Capso and CleanShot X apps, CleanShot settings, capture UI, and user pasteboard were untouched. Next loop: OVL-01b2b native drag-out.

## Loop 38 — Native Quick Access drag-out
**Date:** 2026-08-08 · **Phase:** P2 / OVL-01b2b · **Outcome:** done; partial OVL-01 evidence

Objective: Dragging the exact current Quick Access thumbnail initiates a native macOS copy drag with a bounded screenshot preview and friendly local-time filename while the protected UUID original remains unchanged; stale, released, re-pressed, same-path, and concurrent sessions are rejected and auto-dismiss pauses.

The overlay now arms only a primary-pointer gesture that moves at least six logical pixels. It pauses its remaining eight-second timeout, disables competing actions, and asks Rust to start one native AppKit drag for the exact path and presentation ID. Completion events carry that same identity, so a delayed cancel or drop from an older presentation cannot clear a newer action even when the path repeats. Dropped and cancelled outcomes restore the timer path and report concise status without dismissing durable pixels.

Rust revalidates the exact current capture, fully decodes its PNG off the UI thread, bounds the native preview to 180×112, and creates an actual byte copy with a local-time `Capso YYYY-MM-DD at HH.mm.ss.png` name under a UUID cache directory. It never hands another app the protected Application Support UUID file or a hard link. Cancelled and failed sessions drop and clean the proxy; successful drops retain it for destination apps that read asynchronously, then conservative next-launch cleanup removes only owned single-PNG UUID directories. Copy-only `NSDragOperationCopy` prevents a destination from claiming move semantics.

The final gesture contract snapshots both left-button state and Core Graphics' left-mouse-down event counter before preview work. Native start requires the button still down and the counter unchanged. Releasing, or releasing and pressing again elsewhere while decode/copy completes, therefore cannot revive the old drag. Exact runtime single-flight state also blocks concurrent starts and ignores stale completion callbacks.

Verification: Rust **84/84**, Mac action/gesture/timer **10/10**, web **78/78**, and extension **4/4** tests passed. `cargo fmt --check`, strict Clippy across all targets/features, root typecheck/lint, brand/capture checks, all production builds, a fresh debug Tauri `Capso.app`, loop validator (**67 checks + 7 malformed fixtures**), and `git diff --check` passed. Browser-client dark-mode QA confirmed the overlay structure, visible/enabled Close/Copy/Save controls, and no console errors; browser security policy blocked an embedded forced 252×194 viewport, so no unsupported workaround was used. The installed Capso process remained untouched, and the fresh duplicate bundle was not launched.

Checker: the first review rejected a release-then-repress race because a boolean button check could mistake a later click for the original gesture. Repair attempt 2 added the Core Graphics event-counter identity and deterministic adversarial test. The independent Checker then **APPROVED** with no remaining P0–P2 findings. Implementation commit: `db0ab1e`.

OVL-01 remains IN_PROGRESS. Real Finder/AppKit drop and cancel, bounded-preview appearance, local-time filename, auto-dismiss pause/resume, successful proxy retention and relaunch cleanup, source hash before/after, focus preservation, mixed-scale placement, and latency remain manual evidence. Annotate still depends on the durable upload ownership seam. CAP-02b is a foreground manual proof, so the next independent hourly objective is DUR-01a, the restart-safe local queue state machine. The existing `.app`-suffixed identifier and ad-hoc debug bundle remain PKG-01a; Capso is not ready for distribution or CleanShot cancellation.

## Loop 39 — Synced durable local capture queue
**Date:** 2026-08-08 · **Phase:** P2 / DUR-01a · **Outcome:** done; partial DUR-01 and AC-OFF-01 evidence

Objective: Give every successful native capture a crash-safe local handoff for future background learning while preserving the Application Support PNG as the source of truth. Restart must restore FIFO work, exact retries, idempotency, and poison isolation; queue failure must never turn a saved capture into a failed or deleted capture.

The capture command now syncs both the PNG and its containing directory before reporting durable pixels. It immediately hands that exact UUID path to an atomic JSON queue before clipboard and overlay work. Queue updates sync a unique sibling temporary file, rename, then sync the parent directory. Pre-rename persistence failure rolls memory back. A post-rename directory-sync error remains explicit but retains the visible in-memory commit so restart and exact enqueue retries stay idempotent. The tagged capture event carries independent `queue`, `clipboard`, and `overlay` outcomes, while tray text distinguishes locally safe pending work from a queue warning.

The versioned queue stores capture ID/path/source/time plus pending, uploading, retrying, terminal, and uploaded state. Startup validates before mutation, preserves corrupt or inconsistent JSON, marks missing pixels terminal without blocking healthy work, recovers interrupted attempts onto restart-relative 5-second, 30-second, and 2-minute deadlines, and stops automatic work after the fourth attempt. It also reconciles only direct, non-empty canonical UUID PNG regular files left by a crash before handoff, recording an honest `recovered` source and persisting the set once. Uploaded IDs never become claimable again. There is no production transition that removes a protected capture.

TDD began with unresolved queue types, then pinned atomic enqueue/restart FIFO, dedupe, uploaded finality, retry/poison behavior, interrupted recovery, persistence rollback, corrupt-store preservation, and unsafe-path rejection. A missing-pixel restart test first failed because the entire queue refused to open; repair retained the poison record as terminal while allowing healthy FIFO work. Self-review then found a fourth-attempt crash could exceed the ceiling and added a failing proof before fixing it. Checker pass 1 rejected three cross-file crash boundaries: missing PNG/directory fsync, a permanent orphan gap before enqueue, and interrupted attempts bypassing backoff. Repair attempt 1 added fault-injected sync proofs, immediate handoff plus idempotent orphan reconciliation, and exact interrupted-delay coverage. The independent Checker then **APPROVED** with no remaining P0–P2 findings. Implementation commit: `a5c5e80`.

Verification: Rust **98/98** and Mac **10/10** tests passed; strict Clippy across all targets/features, Mac and root typecheck/lint, brand/capture checks, web **78/78** and extension **4/4** tests, all production builds, loop validator (**67 checks + 7 malformed fixtures**), `git diff --check`, and a fresh arm64 debug Tauri `Capso.app` passed. The installed Capso process was already running, so the fresh duplicate bundle was not launched. Bundle inspection still reports the known `.app`-suffixed identifier and ad-hoc linker signature.

DUR-01 remains IN_PROGRESS. This loop deliberately did not add a network uploader, auth, connectivity monitoring, Supabase writes, server jobs, or AI processing. “Sync pending” currently means safely retained on this Mac, not remotely stored. Next independent objective: DUR-01b1, an idempotent drain coordinator with a fake transport and reconnect wake seam; AI-01a authenticated ingest and the human three-capture offline/restart/reconnect drill remain required before background learning or CleanShot cancellation can be claimed.

## Loop 40 — Idempotent queue drain coordinator
**Date:** 2026-08-08 · **Phase:** P2 / DUR-01b1 · **Outcome:** done; partial DUR-01 and AC-OFF-01 evidence

Objective: Give the restart-safe local queue one production-compiled, single-flight drain coordinator whose upload boundary can be tested without introducing production network or auth changes. Explicit startup, capture-enqueued, connectivity-restored, credentials-restored, and retry-deadline wakes must preserve FIFO, exact capture identity, attempt accounting, and local originals.

The coordinator claims and settles each durable item through short queue operations, then calls its transport outside the queue lock. Only a confirmed acknowledgement containing the exact claimed capture UUID can mark that record uploaded. A mismatched acknowledgement or retryable failure enters the existing persisted 5-second/30-second/2-minute schedule while later healthy work continues. An offline or credentials hold before claim consumes no attempt; a hold discovered during upload durably returns the item to pending and refunds that claim's attempt. No completion path deletes the Application Support PNG.

An atomic single-flight handoff coalesces overlapping wake requests. The active runner clears its ownership before atomically claiming any pending follow-up pass, so transport calls never overlap. Checker pass 1 found that a queue error used the early `?` path and could strand a wake that arrived during that pass. Repair attempt 1 added a deterministic blocked-claim/error overlap proof, routed errors through the same handoff, ran the coalesced follow-up, and then returned the original error. The independent Checker then **APPROVED** with no remaining P0–P2 findings. Implementation commit: `b3b9641`.

TDD began with unresolved coordinator symbols. The final six focused tests prove offline zero-attempt behavior followed by FIFO reconnect, exact-ID rejection, retry isolation, mid-upload credential hold/refund, single-flight overlap, and the repaired error handoff. Restart reopens three completed records without reclaiming them, and every original fixture remains. Verification passed with Rust **104/104**, strict Clippy across all targets/features, full root tests/typecheck/lint/build, loop validator (**67 checks + 7 malformed fixtures**), `git diff --check`, and a fresh debug Tauri `Capso.app`; its executable and `Info.plist` lint passed. The known `.app`-suffixed bundle-identifier warning remains PKG-01a.

DUR-01 remains IN_PROGRESS. The current app instantiates the coordinator but intentionally wires no HTTP/Supabase transport, credentials, connectivity monitor, retry timer, server job, or background AI. The fake transport is not production upload and is not the real AC-OFF-01 three-capture offline/restart/reconnect drill. “Sync pending” still means safely retained on this Mac, not remotely stored. The next independent objective is AI-01a, the Mac identity/auth handoff and authenticated ingest contract; production auth changes remain an owner gate. CleanShot cancellation is not ready.

## Loop 41 — Native PKCE and authenticated-ingest contract
**Date:** 2026-08-08 · **Phase:** P0 auth seam + P2 upload contract / AI-01a1 · **Outcome:** done; partial AI-01a, DUR-01, AC-CAP-03, and AC-OFF-01 evidence

Objective: Compile the security and payload boundary required before the Mac can obtain a durable identity or send its restart-safe queue to Capso. The Mac must originate a single-use S256 PKCE handoff, accept only its exact callback, keep tokens out of URLs/logs/payloads, and define an authenticated ingest request whose ownership comes only from the future bearer session. This loop must not change production auth, migrate anonymous users, register deep links, write Keychain, call the network, mutate Supabase, or claim background learning.

The native runtime now generates high-entropy state/verifier pairs, retains the verifier only in Rust memory, exposes the S256 challenge, and accepts one literal `capso://auth/callback?code=…&state=…` within five minutes. Raw controls, URL-normalized path tricks, wrong scheme/host/path/state, unknown or duplicate query fields, fragments, URL tokens, expiry, and replay are rejected without allowing an invalid callback to consume a still-valid handoff. Authorization codes remain bounded opaque printable data rather than being mistaken for PKCE tokens, while verifier/code debug output is redacted.

The shared Zod and Rust ingest boundaries consume one strict fixture. A request contains a client-stable screenshot UUID, `originals/<owner>/<screenshot-id>.png`, seconds-bearing RFC 3339 time, native source including fullscreen, lowercase SHA-256, annotation flag, and bounded dimensions/bytes—never a caller `user_id`. Rust validates the same canonical UUID/path/hash/time/size limits. Only the exact capture acknowledgement completes future queue work. Unauthorized and Storage-quota responses become no-attempt holds, retryable responses preserve retry semantics, and other contract failures are terminal. Shared negative boundaries cover separator normalization, omitted seconds, invalid hour/leap second/offset, and empty/overlong API messages in both languages.

Checker pass 1 rejected token-shaped OAuth-code assumptions, Rust/Zod timestamp/error drift, and contradictory owner-path wording. Repair 1 made codes opaque printable ASCII, aligned the 100,000-pixel and 1–512-message bounds, introduced shared negative fixtures, and fixed the path language. Checker pass 2 found Zod's omitted-seconds allowance and `url::Url` normalization before raw-shape validation. Repair 2 required seconds in both profiles and rejected raw controls or any pre-query text other than the exact callback URI before URL parsing. The independent Checker then **APPROVED** with no remaining P0–P2 findings. Implementation commit: `c3278ba`.

Verification passed with Rust **113/113**, shared **3/3**, Mac **10/10**, web **78/78**, and extension **4/4** tests; strict Clippy across all targets/features; full workspace typecheck/lint/build; brand/capture generation checks; loop validator (**67 checks + 7 malformed fixtures**); `git diff --check`; and a fresh arm64 debug `Capso.app` whose executable and `Info.plist` passed inspection. The known `com.capso.app` bundle-identifier warning remains PKG-01a.

AI-01 and DUR-01 remain incomplete. The auth and ingest modules are compiled seams but deliberately have no Tauri deep-link registration/command, browser sign-in route, Supabase exchange, Keychain session, HTTP/Storage adapter, connectivity/retry wake source, database write, processing job, or server worker. The existing anonymous browser library cannot be silently transferred to a desktop identity; that production identity/linking choice remains an owner gate. CleanShot cancellation and browser-independent learning are not ready. The next independent code objective is ANN-01a: reuse the existing four-tool editor from the native overlay and flatten the edited pixels before the eventual upload.

## Loop 42 — Native four-tool annotation (failed Checker gate)
**Date:** 2026-08-08 · **Phase:** P2 / ANN-01a · **Outcome:** failed after two repair passes; uncommitted WIP retained

Objective: Open a native arrow/box/text/irreversible-pixelate editor from Quick Access, keep the pending upload reserved while editing, preserve the first original, atomically replace the canonical PNG, re-copy the flattened pixels, and make history/upload consume that same durable path.

The WIP introduced a shared DOM-free annotation model, a hidden focusable/resizable Tauri editor window, exact path/presentation/session validation, four editing tools with move/delete/undo, PNG-only dimension-matched flattening, first-original protection, two-phase queue annotation recording, overlay cache-busting, and restart recovery for a crash between pixel and queue commits. Direct browser QA passed at 1100×760 in light/dark and 760×540 minimum size; arrow and pixelate rendered, PNG flatten produced 75–77 KB payloads, controls did not overflow, and the console was clean. A fresh arm64 debug `Capso.app` bundled successfully. Rust reached **127/127** and workspace tests **97/97**; strict Clippy, format, typecheck, lint, full builds, brand/capture checks, loop validator (**67 checks + 7 malformed fixtures**), and `git diff --check` passed.

Checker pass 1 rejected strict Clippy, close/retry stranding, background auto-dismiss, and missing parent-directory durability. Repair 1 fixed those with a request object, keep-open close disposition, session-long overlay protection, fresh save/cancel presentations, two-phase queue release, commit-visible write errors, and hierarchy sync tests. Checker pass 2 found capture/history publication could replace the overlay during save and that visible originals were not resynced on retry. Repair 2 added capture↔annotation exclusion, atomic overlay+annotation publication guards, and mandatory original-file/parent/directory resync before canonical replacement.

The final Checker still rejected one P1: Recent Captures activates its clipboard ticket before entering the overlay+annotation critical section. If Annotate wins after the early check, history publication is refused but the history ticket remains current; annotated Save and later Copy then fail as stale. Per the two-repair limit, this run stopped without a third fix and did not commit the rejected tree. Next hourly run must make history clipboard activation plus overlay publication transactional—with rollback or deferred ownership—add a deterministic losing-history-versus-Annotate interleaving test, rerun the full gate, and resubmit ANN-01a. CleanShot cancellation remains not ready.

## Loop 43 — Transactional history repair and native annotation approval
**Date:** 2026-08-08 · **Phase:** P2 / ANN-01a · **Outcome:** done; partial ANN-01 and OVL-01 evidence

Objective: Repair the final ANN-01a Checker finding so Recent Captures changes clipboard ownership only when its overlay publication wins against Annotate, prove the losing interleaving deterministically, and resubmit the complete native four-tool editor without narrowing its original behavior.

The clipboard restore transaction now keeps its runtime lock through overlay and annotation arbitration but defers `latest` ticket activation until `prepare_overlay` returns `Prepared`. Any active annotation, transition failure, or event-delivery failure returns a publication error before clipboard identity changes. Annotated Save and refreshed Quick Access Copy therefore retain the exact fresh-capture ticket when a competing history restore loses. Fresh capture registration remains serialized against the history publication boundary.

The new adversarial test pauses history inside publication, activates a real `AnnotationRuntime`, releases history, and proves three outcomes together: history returns `overlay_annotation_active`, the original fresh ticket is unchanged and copyable, and the history path remains stale. This joins the retained native editor WIP: shared DOM-free arrow/box/text/pixelate rendering, focusable Tauri editor, move/delete/undo and palette controls, dimension-matched PNG flattening, first-original durability, queue reservation/two-phase annotation metadata, restart recovery, overlay cache-busting, and flattened-pixel clipboard re-copy.

Verification passed with Rust **128/128** and workspace **97/97** tests (web 79, Mac 11, extension 4, shared 3); `cargo fmt --check`; strict Clippy across all targets/features; root test/typecheck/lint/build; brand and capture generation checks; loop validator (**67 parsed checks + 7 rejected malformed fixtures**); `git diff --check`; and a fresh debug Tauri `Capso.app`. The retained UI had already passed direct light/dark 1100×760 and dark 760×540 browser QA with all four tools, move/delete/undo, destructive pixelation, 75–77 KB PNG flattening, no overflow, and no console errors. The transaction-only repair did not change that UI.

Checker: submission 1 **APPROVED** with no P0–P2 findings. Implementation commit: `42fcfbf`.

Deviation: this automatic continuation inherited Loop 42's rejected working tree and began the targeted repair before reacquiring the canonical hourly lease. The omission was detected before Checker dispatch; the repository had no active lease or concurrent owner, this run then acquired its unique lease, and the late boundary is recorded here rather than hidden. No protected user path or CleanShot setting was touched.

ANN-01 remains IN_PROGRESS because physical native editor/save/copy/relaunch evidence and an exact native redaction/flattened-pixel proof remain. Production upload and browser-independent learning are still blocked by the owner identity/linking decision and unwired auth/transport/worker path. The next independent objective is ANN-01b. Capso is not yet ready for CleanShot cancellation.

## Loop 44 — Exact redaction and flattened-pixel chain
**Date:** 2026-08-08 · **Phase:** P2 / ANN-01b · **Outcome:** done; partial ANN-01 evidence

Objective: Prove with exact pixels—not UI appearance alone—that production redaction destroys source detail and that the flattened PNG remains the canonical artifact through editor input validation, local save, clipboard delivery, durable queue recovery, and upload-drain consumption.

A shared 4×4 opaque grayscale fixture contains 16 distinct source values under one redaction. The proof calls the production shared `pixelate()` implementation with its real default 12-pixel block and requires the exact resulting RGBA bytes: every source value becomes grayscale 85 with alpha 255, unique colors collapse from 16 to 1, and none of the original detail values survives. The native test reads the same fixture, encodes the original and flattened PNGs, sends the flattened data URL through production annotation validation, and persists it through the production flatten path while retaining the exact first original.

The native chain then proves the canonical PNG decodes to the exact flattened pixels, the protected original decodes to the exact source pixels, and the clipboard seam receives the exact canonical PNG bytes. It models a crash after pixels and clipboard commit but before the queue annotation bit, reopens the durable queue, reconciles annotation metadata from disk, and drains through the production coordinator with a fake transport that consumes the same exact flattened canonical bytes. Both the canonical flattened image and protected original still survive after acknowledgement at the drain boundary.

Verification passed with Rust **129/129** and workspace **98/98** tests (web 79, Mac 11, extension 4, shared 4); `cargo fmt --check`; strict Clippy across all targets/features; root test/typecheck/lint/build; brand and capture generation checks; loop validator (**67 parsed checks + 7 rejected malformed fixtures**); fixture validation; `git diff --check`; and a fresh arm64 debug `Capso.app` whose `Info.plist` passed inspection. The known `com.capso.app` identifier warning remains PKG-01a. Checker submission 1 **APPROVED** with no blocking findings. Implementation commit: `4651859`.

This is exact local and drain-boundary evidence, not proof of a real authenticated upload or downloaded stored cloud object. Physical native arrow/box/text/pixelate, save/copy/relaunch, and remote-object comparison remain required before AC-ANN-01 can pass. Production auth/upload/worker work remains owner-gated or unwired. The next independent objective is HIS-01a: thumbnail-backed Recent Captures plus a full-library deep link. Capso is not yet ready for CleanShot cancellation.

## Loop 45 — Queue-timestamped thumbnail history
**Date:** 2026-08-08 · **Phase:** P2 / HIS-01a · **Outcome:** done; partial HIS-01 evidence

Objective: Complete the native Recent Captures menu as a queue-timestamped visual history with exact local restore and an explicit production-library deep link.

The history menu now takes capture time from the durable queue rather than mutable PNG modification time, so saving an annotation cannot make an older screenshot jump to the top. If queue metadata is unavailable, direct local files remain recoverable through the prior modification-time fallback. Candidates are still restricted to direct regular canonical UUID PNGs; each accepted row must fully decode and now carries a native 48×32 aspect-preserving, transparent-letterboxed thumbnail. Sorting happens before decode and stops after five valid entries.

Selecting a recent row retains the existing exact UUID re-resolution, full-decode check, cursor-display Quick Access restore, atomic history/fresh-capture publication, and unchanged pasteboard until Copy. A separate **Open Library…** item opens only `https://capso-cyan.vercel.app/library` after an explicit menu action; opening or constructing the menu performs no network request. Opener failures surface through the tray. The production route returned HTTP 200 during verification.

TDD began with the history suite failing on the intentionally missing queue-backed scan, thumbnail, and opener seams. Verification passed with Rust **133/133** and workspace **98/98** tests (web 79, Mac 11, extension 4, shared 4); `cargo fmt --check`; strict Clippy across all targets/features; root test/typecheck/lint/build; brand and capture generation checks; loop validator (**67 parsed checks + 7 rejected malformed fixtures**); `git diff --check`; and a fresh arm64 debug `Capso.app` whose executable and `Info.plist` passed inspection. The known `com.capso.app` warning remains PKG-01a. Checker submission 1 **APPROVED** with no P0–P2 findings and no repair.

HIS-01 remains IN_PROGRESS because native relaunch/menu thumbnail/click/focus behavior and queued end-to-end cloud persistence are not yet physically proven. No Capso or CleanShot capture mode was launched, no clipboard or settings changed, and no production state was written. The next ordered code item is PKG-01a, but the project STOP rule requires Elvin to approve the permanent reverse-DNS bundle identity and entitlement manifest first. AI-01a also remains gated on whether the native identity adopts/migrates the existing anonymous browser library or starts fresh. Capso is not yet ready for CleanShot cancellation.

## Loop 46 — Native overlay speed evidence
**Date:** 2026-08-08 · **Phase:** P2 / CAP-02b1 · **Outcome:** done; instrumentation only

Objective: make CAP-02b objectively measurable during daily use by recording and surfacing the latest 20 successful real capture-to-visible-overlay durations without storing screenshot identity, path, time, or pixels.

Timing starts immediately after the native `/usr/sbin/screencapture` process returns and ends only after the exact image has decoded in the overlay webview and the native show call succeeds. That boundary deliberately includes metadata validation, file and directory durability, queue handoff, clipboard delivery, frontend decode, and native presentation. It is the earliest robust command-line boundary available without Accessibility monitoring; it does not claim to measure raw mouse-release time. Only a successful fresh exact presentation records once. Stale callbacks, history restores, annotation refreshes, decode failures, and native show failures are excluded.

The native runtime atomically persists only capture mode plus duration in `$APPDATA/overlay-latency.json`, keeps the latest 20 across restart, recovers safely from missing/corrupt evidence, and surfaces a generic warning without revealing capture data. The tray's **Overlay Speed Check** reports progress plus nearest-rank p50, p90, and maximum values. PASS requires exactly 20/20 latest samples strictly below 1,000 ms; 19 fast samples cannot pass.

TDD began with unresolved latency/runtime symbols and then missing capture-start/reveal seams. The final targeted proofs cover the exact process-return boundary, privacy and restart behavior, exact percentile math, strict 19/20 and 20×999 ms claims, corrupt-store recovery, and exclusion of stale, duplicate, history, annotation, decode-failed, and show-failed presentations. Checker submission 1 **APPROVED** with no P0–P2 findings. Implementation commit: `ec43534`.

Verification passed with Rust **140/140** and workspace **98/98** tests (web 79, Mac 11, extension 4, shared 4); `cargo fmt --check`; strict Clippy across all targets/features; root typecheck/lint/build; brand and capture-generation checks; loop validator (**67 parsed checks + 7 rejected malformed fixtures**); and `git diff --check`. A fresh arm64 debug `Capso.app` bundled successfully with the new speed-check strings; its executable and `Info.plist` passed inspection. The existing temporary `com.capso.app` identity and ad-hoc signature remain PKG-01 blockers. A full DMG attempt did not produce a final artifact, so this loop claims only the fresh app bundle.

CAP-02 remains IN_PROGRESS. The foreground 20-capture perceived-latency run, real general-pasteboard copy/paste behavior, focus preservation, and mixed-scale display behavior remain manual evidence. No Capso or CleanShot capture mode was launched, no clipboard/settings/permissions or production state changed, and no network/auth/analytics path was added. Next: run the bundled app through the physical CAP-02b check, or resolve the permanent bundle identity and anonymous-library linking owner gates before the corresponding code can continue. Capso is not yet ready for CleanShot cancellation.

## Loop 47 — Browser-independent one-job classification worker

**Date:** 2026-08-08 · **Phase:** P1/P3 / AI-01b1 · **Outcome:** done; local worker core only

Objective: add a locally testable Supabase Edge worker that claims no more than one persisted `process_capture` job, reads the exact stored PNG plus owner-scoped projects and recent project corrections, calls MiniMax without a browser, and atomically settles the classification and job. Production migration apply, function deployment, secrets, Vault/Cron, job creation, native auth/upload, embeddings, and model/live-database calls were explicitly outside this slice.

The new unapplied migration adds an RLS-readable, worker-mutated jobs table and three service-role-only `SECURITY DEFINER` RPCs. Claiming uses `FOR UPDATE SKIP LOCKED`, one active processing lease per owner, exact worker identities, a ten-minute reclaim boundary, and paid-call attempt accounting. Already-processed/deleted work becomes done without a model call; stale exhausted leases become failed rather than receiving an extra call. Completion locks the exact job and screenshot, validates project ownership, writes structured classification/search/routing fields, and marks the job done in one transaction. Failure persists only generic codes, retries at 5s/30s/2m, and leaves terminal screenshots visibly unprocessed.

The Edge handler accepts POST only and performs a constant-time, minimum-32-character worker-secret check before repository access. The repository accepts only the claimed `owner/screenshot.png` identity, streams a maximum 20 MB PNG with signature verification, retrieves at most 50 active projects and 20 recent project corrections, and never puts the service-role key in a URL. Correction joins require both correction and screenshot owner equality and are checked again at runtime. Context responses stop at 512 KB, individual fields are UTF-8 bounded, and the final model prompt cannot exceed 48 KB. The MiniMax adapter sends the exact base64 pixels to the Anthropic-compatible endpoint with a 55-second timeout; provider bodies, reasoning, credentials, paths, and screenshot metadata are never persisted as errors. The shared Zod classification contract now has a dependency-light export used by both browser and Deno paths.

Checker submission 1 **REJECTED** two P1 findings: a service-role correction embed could read a cross-owner screenshot if a malformed correction row carried the caller's `user_id`, and unbounded PostgreSQL text could exhaust Edge/provider prompt limits. Repair 1 added an inner owner-equality filter plus runtime owner check, bounded streaming for image/context/RPC bodies, per-field UTF-8 truncation, a total prompt ceiling, and direct hostile-context tests. Final resubmission **APPROVED** with no remaining P0–P2 findings; PostgreSQL execution and hosted deployment remain explicitly unverified because no local Docker/Postgres runtime was available and production changes require owner approval.

Verification passed with Edge worker **18/18** and existing workspace **98/98** tests (web 79, Mac 11, extension 4, shared 4); `deno fmt --check`; `deno check`; `deno info` (**6 modules, 3.87 MB**); root typecheck/lint/build; brand and capture-generation checks; loop validator (**67 parsed checks + 7 rejected malformed fixtures**); and `git diff --check`. Current official Supabase guidance still supports per-function `deno.json` and monorepo imports; no relevant Edge/PostgREST breaking change was found.

AI-01 remains BLOCKED as an end-to-end gate. This loop did not apply or syntax-run the migration against Postgres, call MiniMax, deploy the function, create a job producer, configure Vault/Cron, establish a native session, upload pixels, generate embeddings, or prove a capture processes with every browser closed. No Capso or CleanShot capture mode was launched and no clipboard, permission, CleanShot, credential, production, or distribution state changed. Next: owner-approved production integration plus AI-01a Mac auth/upload/job creation, then AI-01c no-browser proof. Capso is not ready for CleanShot cancellation.

## Loop 48 — Authenticated native startup and capture drain

**Date:** 2026-08-09 · **Phase:** P0/P2 / AI-01a2 + DUR-01b2 · **Outcome:** done; APPROVED after one Checker repair

Objective: turn the locally verified PKCE/Keychain and Storage/RPC seams into one production-compiled native runtime without changing hosted Supabase. When the same-account session already exists in macOS Keychain, startup and each durably queued capture must refresh it when needed and drive the real queue coordinator off the UI thread. Missing or unsafe public configuration and missing sessions must hold the queue without claiming an attempt.

The new session coordinator reads the Keychain repository, reuses a fresh access token without an Auth request, or performs bounded refresh-token rotation and saves the complete replacement session before returning credentials. The production sync runtime accepts only an HTTPS project URL paired with an `sb_publishable_` key embedded at build time; partial configuration and server-secret-shaped keys fail closed without logging values. A missing configuration creates a local-only disabled runtime rather than a fake upload path.

The runtime now composes that session coordinator with the existing bounded authenticated Storage/RPC transport and single-flight durable drain. Tauri owns one serialized runtime, wakes it once after startup, and wakes it again only after a capture is durably enqueued or already queued. Work runs through `spawn_blocking`, so capture and menu UI do not wait for Keychain, Auth, Storage, or RPC I/O. A cancelled capture, failed capture, or queue handoff failure never triggers a drain.

Checker submission 1 **REJECTED** one P1 race: the immediate authenticated wake could claim and upload a fresh capture before the user clicked Annotate. Repair 1 atomically reserves every runtime enqueue for Quick Access, transfers that hold into the annotation session, and excludes both hold types from claims without consuming an attempt. Overlay replacement, dismissal, decode/show/publication failure, and completed annotation release the exact hold and schedule a coalesced wake; annotation cancel restores the Quick Access hold. A fast authenticated integration test proves the first wake makes zero attempts and the post-annotation wake reads the exact flattened PNG bytes.

The same independent Checker **APPROVED** repair 1 with no remaining P0–P2 findings. The complete final source, tests, and outcome records were then staged together for the required exact-diff digest attestation before commit.

TDD began with unresolved session-coordinator, sync-runtime, public-config, and capture-wake symbols, then reproduced the Checker race with an authenticated fast transport before repair. Focused tests now prove network-free fresh-session reuse, refresh rotation persistence, missing-session zero-network behavior, Quick Access/annotation zero-claim holds, the exact production drain boundary, public-config rejection, and exact capture wake selection. Verification passed with Rust **160/160**, workspace **98/98**, and worker **19/19** tests; `cargo fmt --check`; strict Clippy across all targets; root typecheck/lint/build; fake native secret rejection; loop validation; and `git diff --check`. A fresh **46 MB arm64** debug `Capso.app` bundled with the expected Supabase project host and valid `Info.plist`; it remains ad-hoc linker-signed with the known temporary identity and was not launched or installed.

No production auth setting, schema, migration, function, secret, or schedule changed. Safe hosted inspection found Auth reachable, email sign-in enabled, anonymous sign-in disabled, and the public Data API schema unavailable under the configured publishable key. Therefore neither same-account website/Mac synchronization nor background learning is claimed yet: Capso still needs login/deep-link session creation, an owner-approved identity/linking decision, hosted schema/function apply and proof, timed/connectivity retry wakes, and the three-capture offline/restart/reconnect drill. The pasted Supabase secret/password must be rotated before production use.

## Loop 50 — Native email sign-in without capture blocking

**Date:** 2026-08-10 · **Phase:** P0/P2 / AI-01a3 · **Outcome:** done; product review APPROVED

Objective: resume the quarantined cumulative native-auth slice, connect first-session creation through email OTP and a strict token-free browser handoff, and close the rejected concurrency defect by proving stalled Auth HTTP cannot prevent a new capture transition from entering. The resumed source lineage is retained at local quarantine ref `refs/capso-quarantine/manual-ai01a3-bb748493-aa09-48b7-aa51-17582dc05cb2`; rejected outcome records were not restored.

The native account panel now requests an email OTP with the public publishable key, generates PKCE S256 plus an expiring one-time state, and opens only the configured HTTPS `/auth/open-capso` handoff. The locally build-verified Next route accepts exactly `code` and `state`, adds no-store/CSP/referrer/nosniff headers, and emits only `capso://auth/callback?code=...&state=...`; access and refresh tokens never enter either URL. The native deep-link handler performs the bounded PKCE exchange, rejects stale/replayed/mismatched callbacks and different-user replacement, persists the complete session in macOS Keychain, and wakes the existing authenticated queue drain. Account status and callback failures are surfaced in the app, and sign-out is refused while uploadable work remains.

The final repair replaces the broad auth-transition lock with short, explicit state transitions. RAII counters track active Auth operations and queue drains without retaining the transition mutex during OTP/PKCE/status/session-read/refresh, Storage, or RPC work. Capture enqueue uses only the short mutex boundary. Sign-out atomically refuses deletion whenever auth/drain work is active or pending/uploading/retrying records remain, then intentionally retains that boundary while deleting the Keychain session. Two production-path blocking tests stall the OTP request and PKCE exchange independently and prove the capture transition can enter before either HTTP request resumes; durable enqueue independence follows structurally from queue enqueue taking only that same short boundary and is not directly exercised by those two tests.

TDD first failed on the missing active-operation guard, then passed with Rust **170/170**, workspace **101/101** tests (web 82, Mac 11, extension 4, shared 4), and Edge worker **19/19**. `cargo fmt --check`; strict Clippy across every target/feature; root typecheck, lint, brand/capture checks, and production builds; Deno test/check; loop validation (**67 parsed checks + 7 rejected malformed fixtures**); and diff checks pass. A fresh arm64 debug bundle produced a **47 MB `Capso.app`** and **13 MB DMG**. Its plist contains the `capso` callback scheme, its executable contains no `sb_secret_` or `service_role` markers, and it remains intentionally ad-hoc linker-signed with the known temporary bundle identity. It was not launched, installed, or distributed.

The same independent Checker **APPROVED** the complete product submission with no remaining P0–P2 findings and confirmed the auth/network lock boundaries, queue/sign-out serialization, different-user protection, strict token-free handoff, and truthful unsynced UI copy. The exact final staged source plus planning/outcome records still require digest attestation before commit.

No route was deployed, no hosted Auth redirect was allowlisted, no database migration/function/secret/Cron was changed, and no production model call occurred. The web app remains anonymous/local rather than demonstrably the same email account, so cross-device synchronization and background learning are not yet claimed. Next independent code objective: DUR-01b3 timed/connectivity retry wakes. Owner decisions remain required for anonymous-library migration versus a fresh authenticated library, hosted redirect/data changes, permanent bundle identity, signing/notarization, and distribution. The previously pasted Supabase credential must be rotated before production use.

## Loop 51 — Persisted retry deadline and reconnect wakes

**Date:** 2026-08-10 · **Phase:** P2 / DUR-01b3 · **Outcome:** done; APPROVED after one Checker repair

Objective: wake the authenticated native upload drain at persisted retry deadlines and within five seconds of a known macOS offline-to-online route transition, without consuming queue attempts while the route is known offline and without imposing a permanent idle poll.

The native queue exposes one locked monitor snapshot containing whether retryable work exists and its earliest persisted deadline. A background monitor uses SystemConfiguration reachability, preserves the last known state when a probe itself fails, and passes known-offline wakes into the existing pre-Auth/pre-transport hold. Retry deadlines wake the same single-flight coordinator; an unchanged overdue deadline can fire again after a bounded 30-second hold so an Auth failure before queue claim cannot strand the item. Each background-sync completion signals the monitor to observe a newly persisted deadline or return to sleep.

The monitor uses a bounded, coalescing event channel. With no retryable work—or online work without a deadline—it blocks for a real queue/auth event rather than waking every five seconds. A pending known-offline item or persisted deadline retains at-most-five-second route observation, and a fresh background sync performs its own off-UI-thread route probe before touching Auth or transport. Unknown reachability fails open; known offline fails closed with zero claims.

Checker submission 1 **REJECTED** three findings: a pre-claim Auth failure could permanently suppress an unchanged overdue deadline, a reachability error erased the offline state required to classify the later online probe as `Restored`, and the initial monitor polled forever even with an empty queue. Repair 1 added bounded rearming plus the exact Auth-before-claim regression, error-preserving connectivity transitions, and queue-aware event sleeps. The same independent Checker **APPROVED** the repair with no remaining P0–P2 findings.

TDD first failed on the missing connectivity, deadline, queue-snapshot, and monitor-wait seams. Verification passed with Rust **179/179** and workspace **101/101** tests (web 82, Mac 11, extension 4, shared 4); `cargo fmt --check`; strict Clippy across every target/feature; root typecheck, lint, brand/capture checks, and production builds; Edge worker **19/19** tests plus Deno check; loop validation (**67 parsed checks + 7 rejected malformed fixtures**); and `git diff --check`. A fresh arm64 debug bundle produced a **47 MB `Capso.app`** and **13 MB DMG**; SystemConfiguration is linked, the executable contains no `sb_secret_` or `service_role` marker, and it remains intentionally ad-hoc linker-signed with the temporary identity. A current Deno formatter would reflow one untouched worker migration test; it is outside this objective and has no source diff, so the slice did not rewrite it.

No app or DMG was launched, installed, or distributed. No route was deployed, no hosted Auth or database setting changed, no migration/function/secret/Cron was applied, and no production model call occurred. DUR-01 remains incomplete until hosted ingest/processing is approved and proven and the physical three-capture offline/restart/reconnect drill passes. Cross-device synchronization still requires hosted identity/data proof, and CleanShot cancellation still requires the remaining physical, five-day dogfood, permanent bundle identity, signing, notarization, and credential-rotation gates.

## Direct build — Fresh authenticated website library

**Date:** 2026-08-10 · **Phase:** P0 / AI-01a4 (local source only) · **Outcome:** locally verified; not deployed

The owner chose a fresh authenticated library. The configured web app now requires a permanent email magic-link session before mounting its store, uses the same Supabase user id for every remote row/object, clears legacy anonymous sessions with browser-local sign-out, and refuses to turn auth/backend failures into a misleading IndexedDB library. Unconfigured developer builds remain local-only. Account status and local sign-out are connected in settings; anonymous-library import is no longer offered in the signed-in experience.

TDD began with four expected failures for the missing account module. The final web suite passes **86/86** tests, plus web typecheck, lint, and production build. Real-browser QA at 1440×1000 confirmed the sign-in page names the same-email Mac/web contract and fresh-library consequence with no console errors. No OTP was sent, no production route/Auth/schema/function/secret/Cron was changed, and nothing was deployed or pushed. Hosted redirect/website deployment, live same-account data proof, API auth hardening, worker integration, and physical offline QA remain.

## Direct build — CleanShot parity extras, server boundary, CI, and hosted deployment reconciliation

**Date:** 2026-08-11 · **Phase:** P0/P2 mixed · **Outcome:** locally verified; hosted infrastructure confirmed live

This entry records the 2026-08-10→11 working session committed on 2026-08-11. Mac: a one-pin always-on-top pinned capture window (`pin.rs`, generation-guarded, hide-not-destroy close, not restart-persistent by design), a cancellable five-second region self-timer (`self_timer.rs`, countdown window, tray item), an explicit CSP replacing `"csp": null` in `tauri.conf.json` guarded by `csp.check.ts`, and history widened from five to eight recents. Web: an authenticated owner-scoped DELETE at `api/screenshots/[id]` backed by `lib/api/{auth-guard,delete-screenshot,rate-limit}.ts` and `lib/supabase/server-auth.ts`, deriving storage object paths from the owner's DB row rather than trusting the client; canonical bucket-prefixed storage paths with legacy fallback; a `BackendPreference` auto/local switch. CI: a new `.github/workflows/quality.yml` (node/web + Playwright, Deno worker, macOS Rust) with pinned toolchains (`.node-version` 22.14.0, `.dvmrc` 2.6.7, `rust-toolchain.toml` 1.92.0) and root `verify`/`test:*` scripts — this workflow has never executed and its first push is its first run.

Supabase migrations were renamed to CLI timestamp convention (four byte-identical renames; `background_processing_jobs` gains a head-of-queue join so an upload-pending row cannot block later eligible work; `authenticated_native_ingest` gains `p_thumb_path` with an expected-path assertion) and five new migrations were added (`move_vector_out_of_public`, `harden_rls_and_grants`, `cover_owner_foreign_keys`, `fix_claim_job_ambiguity`, `schedule_screenshot_worker`).
