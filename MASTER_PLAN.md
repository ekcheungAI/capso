...
| D17 | 2026-08-12 | **The illustration ban is reversed; the character ban is not.** Capso ships a generated visual language — direction "Decks and Trays", still-life photography of capsule lids in decks and paperboard trays — across empty states, first-run, marketing and Mac. Supersedes `15_DESIGN_SYSTEM_AND_UX.md` §"Empty states" ¶*no illustrations*, which conflated the two bans. Guardrails that survive: no character/face/eyes, no sparkles or decoration, and art may never out-saturate a capture (chroma ceilings live in `tokens.json` `illustration` and are asserted by `drafts/brand/art/build_art.py`, not eyeballed). Sample screenshots are a separate off-brand class and depict **fictional** products only — the seed fixtures were renamed off Linear/Stripe/Notion/Superhuman/Raycast/Arc in the same loop, because a photoreal image captioned with a real competitor's name is a fabricated depiction rather than an annotation. |
