...
- [ ] Local upload queue: persist to disk (SQLite or JSON queue), retry on failure, survives app restart, drains on reconnect (offline support — AC-OFF-01) <!-- partial: a5c5e80 syncs capture pixels before atomic JSON handoff and proves restart FIFO/orphan recovery, exact 5s/30s/2m retry, four-attempt poison isolation, idempotency, corrupt-store preservation, and zero deletion; b3b9641 adds the single-flight coordinator; c3278ba adds the authenticated contract. The real Storage/RPC transport now runs at startup and after durable capture enqueue, and locally connected email/PKCE can create its Keychain session. Missing config/session and Quick Access/annotation holds consume no attempt; auth HTTP does not hold capture transitions. Loop 51 adds persisted-deadline wakes, bounded same-deadline rearming, and an event-driven macOS route monitor that sleeps when idle and wakes within five seconds after a known offline route returns. Hosted proof and the physical three-capture offline/restart/reconnect drill remain -->
- [ ] Upload path: Storage put (original + generated thumbnail) → insert `screenshots` row → enqueue `process_screenshot` job <!-- partial: the macOS client has a bounded JWT-authenticated original-PNG Storage/RPC transport and the unapplied RPC atomically inserts the owner-derived screenshot + process job with exact retry acknowledgement. Startup/capture/deadline/reconnect runtime wakes and local email/PKCE session creation now exist; thumbnail generation, hosted redirect/data configuration, production apply, and live integration remain -->
